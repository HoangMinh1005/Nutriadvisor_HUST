...
for f in scheduler.foods:
    ...
    is_pure_fiber = "role_fiber" in tags and not ("role_protein" in tags or "role_carb" in tags or "is_main_dish" in tags)
    ...
    if "is_main_dish" in tags:
        ...
        if not is_snack and not is_pure_fiber:
            if "allergen_egg" in tags or "role_carb" in tags or "allergen_milk" in tags:
                breakfast_foods.append(fid)
            if "role_protein" in tags or "role_carb" in tags:
                lunch_foods.append(fid)
                dinner_foods.append(fid)
...
print("Total raw daily solutions:", len(sols))
...
# Test all solutions but no print inside the loop to make it fast
for sol in sols:
    try:
        ...
        if plan:
            scaled_count += 1
            if not constraints.check_daily_calories(plan, 1.0):
                cal_fails += 1
                continue
            if not constraints.check_daily_macros(plan, 1.0):
                macro_fails += 1
                continue
            costs = [m["cost_vnd_100g"] for m in plan]
            # Bypassed budget check to analyze remaining constraints
            # if not constraints.check_daily_budget(costs, 1.0):
            #     budget_fails += 1
            #     continue
            if not constraints.check_calorie_distribution(plan, 1.0):
                dist_fails += 1
                continue
...
print(f"Total scaled: {scaled_count}")
print(f"Failed calories: {cal_fails}")
print(f"Failed macros: {macro_fails}")
print(f"Failed budget: {budget_fails}")
print(f"Failed distribution: {dist_fails}")
print(f"Passed all: {scaled_count - cal_fails - macro_fails - budget_fails - dist_fails}")
