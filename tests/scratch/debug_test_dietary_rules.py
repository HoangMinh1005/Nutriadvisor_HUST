...
from csp.scheduler import get_dynamic_tags, get_food_role, is_high_quality_protein
from constraint import Problem

# Copy rich_mock_foods from test_csp_scheduler.py
rich_mock_foods = [
...
# Print slot allocations in _solve
...
print("Breakfast foods:", [scheduler.food_by_id[fid]["name_vi"] for fid in breakfast_foods])
print("Lunch foods:", [scheduler.food_by_id[fid]["name_vi"] for fid in lunch_foods])
print("Snack foods:", [scheduler.food_by_id[fid]["name_vi"] for fid in snack_foods])
print("Dinner foods:", [scheduler.food_by_id[fid]["name_vi"] for fid in dinner_foods])
...
print("Total raw daily solutions found:", len(sols))

# Partition mock foods
all_carbs = []
all_proteins = []
all_fibers = []
all_snacks = []
...
# Let's check how many pass calories and macros
valid_scored = []
found_valid = False
for sol in sols:
    b_food = scheduler.food_by_id[sol["breakfast"]]
    l_food = scheduler.food_by_id[sol["lunch"]]
    s_food = scheduler.food_by_id[sol["snack"]]
    d_food = scheduler.food_by_id[sol["dinner"]]
    ...
    try:
        ...
        if plan:
            cal_ok = constraints.check_daily_calories(plan, 1.0)
            macro_ok = constraints.check_daily_macros(plan, 1.0)
            costs = [m["cost_vnd_100g"] for m in plan]
            budget_ok = constraints.check_daily_budget(costs, 1.0)
            dist_ok = constraints.check_calorie_distribution(plan, 1.0)
            
            if cal_ok and macro_ok and budget_ok and dist_ok:
                print(f"FOUND VALID DAILY PLAN! Sol: {sol}")
                for m in plan:
                    print(f"  {m['meal_type']}: {m['name']} (Cal: {m['calories']}, Cost: {m['cost_vnd_100g']})")
                valid_scored.append(plan)
                found_valid = True
                break
    except Exception as e:
        print(f"Exception for solution {sol}: {type(e).__name__}: {e}")
        break

print("Total valid scaled daily plans:", len(valid_scored))
